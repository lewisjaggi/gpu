...
AmplificationTest2::AmplificationTest2(const Grid& grid, uint w, uint h, int min, int max) :
	Animable_I<uchar4>(grid, w, h, "AmplificationTest2_Cuda_RGBA_uchar4")
    {
    this->min = min;
    this->max = max;
...
AmplificationTest2::~AmplificationTest2()
    {
 ...
 */
void AmplificationTest2::process(uchar4* ptrDevPixels, uint w, uint h, const DomaineMath& domaineMath)
    {
 ...
 * Call periodicly by the API
 */
void AmplificationTest2::animationStep()
{
//rien
}

uchar* AmplificationTest2::testTabGMValueAfterAmplificationUchar()
{
...
float* AmplificationTest2::testTabGMValueAfterAmplificationFloat()
{
    uchar4* fakeImage = new uchar4[w * h];
    uchar4* fakeImageDev;
